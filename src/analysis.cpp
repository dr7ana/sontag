...
namespace sontag {
    namespace detail {
        ...
        struct prepared_exec_cells_for_render {
            std::vector<std::string> cells{};
            std::optional<std::string> selected_return_expression{};
        };
        ...
        static prepared_exec_cells_for_render prepare_exec_cells_for_render(
                const std::vector<std::string>& exec_cells) {
            auto prepared = prepared_exec_cells_for_render{};
            prepared.cells.reserve(exec_cells.size());
            for (const auto& cell : exec_cells) {
                auto render_cell = std::string{cell};
                if (has_effective_code(render_cell)) {
                    // Strip terminal returns from each exec cell and emit a single canonical return for main.
                    if (auto trailing_return = parse_trailing_return_statement(render_cell)) {
                        if (trailing_return_has_expression(render_cell, *trailing_return)) {
                            prepared.selected_return_expression =
                                    std::string{trailing_return_expression(render_cell, *trailing_return)};
                        }
                        render_cell.erase(trailing_return->token_start);
                    }
                }
                prepared.cells.emplace_back(trim_trailing_ascii(render_cell));
            }
        ...
        static std::string render_source(
            ...
            auto prepared_exec_cells = prepare_exec_cells_for_render(exec_cells);
            auto& render_exec_cells = prepared_exec_cells.cells;
            ...
            if (prepared_exec_cells.selected_return_expression) {
                source << "    return {};\n"_format(*prepared_exec_cells.selected_return_expression);
            }
            else {
                source << "    return 0;\n";
            }
        ...
        static constexpr bool ends_with_newline(std::string_view text) noexcept {
            return !text.empty() && text.back() == '\n';
        }

        static std::string render_decl_header(const std::vector<std::string>& decl_cells) {
            std::ostringstream decl{};
            decl << "// generated by sontag m1\n";
            decl << "#pragma once\n\n";
            for (size_t i = 0U; i < decl_cells.size(); ++i) {
                decl << "// decl cell " << (i + 1U) << '\n';
                decl << decl_cells[i];
                if (!ends_with_newline(decl_cells[i])) {
                    decl << '\n';
                }
                if (i + 1U < decl_cells.size()) {
                    decl << '\n';
                }
            }
            return decl.str();
        ...
        static std::string render_exec_translation_unit(const std::vector<std::string>& exec_cells) {
            auto prepared_exec_cells = prepare_exec_cells_for_render(exec_cells);
            auto& render_exec_cells = prepared_exec_cells.cells;

            std::ostringstream source{};
            source << "// generated by sontag m1\n";
            source << "#include \"decl.hpp\"\n\n";
            source << "int main() {\n";
            for (size_t i = 0U; i < render_exec_cells.size(); ++i) {
                source << "    // exec cell " << (i + 1U) << '\n';
                write_exec_cell(source, render_exec_cells[i]);
        ...
        static std::string sanitize_workspace_component(std::string_view value) {
            std::string out{};
            out.reserve(value.size());
            for (char c : value) {
                auto ascii = static_cast<unsigned char>(c);
                if (std::isalnum(ascii) != 0 || c == '_' || c == '-' || c == '.') {
                    out.push_back(c);
                }
                else {
                    out.push_back('_');
        ...
        static std::string make_workspace_root_alias(const fs::path& root, size_t index) {
            auto name = root.filename().string();
            if (name.empty()) {
                name = "root";
            }
            return "{:02d}_{}"_format(index, sanitize_workspace_component(name));
        }

        static void recreate_directory_symlink(const fs::path& link_path, const fs::path& target_path) {
            std::error_code ec{};
            if (fs::exists(link_path, ec) || fs::is_symlink(link_path, ec)) {
                ec.clear();
                fs::remove_all(link_path, ec);
                if (ec) {
                    throw std::runtime_error("failed to clear existing import link: {}"_format(link_path.string()));
                }
            }
            ...
            throw std::runtime_error(
                    "failed to create import symlink {} -> {}"_format(link_path.string(), target_path.string()));
        }

        struct materialized_source_unit {
            fs::path source_path{};
            std::optional<fs::path> decl_path{};
            std::optional<fs::path> exec_path{};
            std::vector<fs::path> import_root_links{};
        };
        ...
        struct file_fingerprint_record {
            std::string path{};
            uintmax_t size{};
            long long mtime_ns{};
            uint64_t dev{};
        ...
        struct content_hash_cache_entry {
            uintmax_t size{};
            long long mtime_ns{};
            uint64_t dev{};
            uint64_t ino{};
        ...
        static std::unordered_map<std::string, content_hash_cache_entry>& import_content_hash_cache() {
            static auto cache = std::unordered_map<std::string, content_hash_cache_entry>{};
            return cache;
        }

        static std::string normalize_path_for_fingerprint(const fs::path& path) {
            std::error_code ec{};
        ...
        static std::optional<file_fingerprint_record> collect_file_fingerprint_record(const fs::path& path) {
            struct stat stat_buffer{};
            if (::stat(path.c_str(), &stat_buffer) != 0) {
                return std::nullopt;
            }

            auto mtime_ns = static_cast<long long>(stat_buffer.st_mtime) * 1000000000LL;
#if defined(__APPLE__)
            mtime_ns += static_cast<long long>(stat_buffer.st_mtimespec.tv_nsec);
#elif defined(__linux__)
            mtime_ns += static_cast<long long>(stat_buffer.st_mtim.tv_nsec);
#endif

            return file_fingerprint_record{
                    .path = normalize_path_for_fingerprint(path),
                    .size = static_cast<uintmax_t>(stat_buffer.st_size),
                    .mtime_ns = mtime_ns,
                    .dev = static_cast<uint64_t>(stat_buffer.st_dev),
                    .ino = static_cast<uint64_t>(stat_buffer.st_ino),
                    .present = true};
        }

        static std::vector<file_fingerprint_record> collect_import_file_fingerprint_records(
                const analysis_import_context& context) {
            auto unique_paths = std::set<std::string>{};
            for (const auto& file_path : context.files) {
                unique_paths.insert(normalize_path_for_fingerprint(file_path));
            }
            if (context.entry) {
                unique_paths.insert(normalize_path_for_fingerprint(*context.entry));
            }

            auto records = std::vector<file_fingerprint_record>{};
            records.reserve(unique_paths.size());
            for (const auto& path_string : unique_paths) {
                auto path = fs::path{path_string};
                if (auto record = collect_file_fingerprint_record(path)) {
                    records.push_back(std::move(*record));
                    continue;
                }
                records.push_back(
                        file_fingerprint_record{.path = normalize_path_for_fingerprint(path), .present = false});
            }

            std::ranges::sort(records, [](const file_fingerprint_record& lhs, const file_fingerprint_record& rhs) {
                return lhs.path < rhs.path;
            });
            return records;
        }

        static std::string compute_cached_import_content_hash(const file_fingerprint_record& record) {
            if (!record.present) {
                return "<missing>";
            }

            auto& cache = import_content_hash_cache();
            if (auto it = cache.find(record.path); it != cache.end()) {
                auto metadata_unchanged = it->second.size == record.size && it->second.mtime_ns == record.mtime_ns &&
                                          it->second.dev == record.dev && it->second.ino == record.ino;
                if (metadata_unchanged) {
                    return it->second.content_hash;
                }
            }

            auto content = read_text_file(record.path);
            auto digest = hash_fnv1a_64_hex(content);
            cache.insert_or_assign(
                    record.path,
                    content_hash_cache_entry{
                            .size = record.size,
        ...
        static std::string render_build_unit_payload(const analysis_request& request, std::string_view source_text) {
            ...
            if (request.import_context) {
                ...
                for (size_t i = 0U; i < request.import_context->roots.size(); ++i) {
                    payload << "import.root_target[" << i
                            << "]=" << normalize_path_for_fingerprint(request.import_context->roots[i]) << '\n';
                }

                auto records = collect_import_file_fingerprint_records(*request.import_context);
                for (size_t i = 0U; i < records.size(); ++i) {
                    payload << "import.fingerprint.file[" << i << "].path=" << records[i].path << '\n';
                    payload << "import.fingerprint.file[" << i
                            << "].present=" << (records[i].present ? "true" : "false") << '\n';
                    if (!records[i].present) {
                        continue;
                    }
                    payload << "import.fingerprint.file[" << i << "].size=" << records[i].size << '\n';
                    payload << "import.fingerprint.file[" << i << "].mtime_ns=" << records[i].mtime_ns << '\n';
                    payload << "import.fingerprint.file[" << i << "].dev=" << records[i].dev << '\n';
                    payload << "import.fingerprint.file[" << i << "].ino=" << records[i].ino << '\n';
                    payload << "import.fingerprint.file[" << i
                            << "].content_hash=" << compute_cached_import_content_hash(records[i]) << '\n';
                }
            }
        ...
        static materialized_source_unit materialize_source_unit(
                const analysis_request& request, const fs::path& inputs_dir, std::string_view source_text) {
            if (!request.import_context) {
                auto source_path = inputs_dir / "source.cpp";
                (void)write_text_file(source_path, source_text);
                return materialized_source_unit{.source_path = source_path};
            }

            auto workspace_dir = inputs_dir / "workspace";
            auto import_dir = workspace_dir / "import";
            ensure_dir(workspace_dir);
            ensure_dir(import_dir);

            auto import_links = std::vector<fs::path>{};
            import_links.reserve(request.import_context->roots.size());
            for (size_t i = 0U; i < request.import_context->roots.size(); ++i) {
                auto alias = make_workspace_root_alias(request.import_context->roots[i], i);
                auto link_path = import_dir / alias;
                recreate_directory_symlink(link_path, request.import_context->roots[i]);
                import_links.push_back(link_path);
            }

            auto decl_path = workspace_dir / "decl.hpp";
            auto exec_path = workspace_dir / "exec.cpp";
            auto decl_text = render_decl_header(request.decl_cells);
            auto exec_text = render_exec_translation_unit(request.exec_cells);
            (void)write_text_file(decl_path, decl_text);
            (void)write_text_file(exec_path, exec_text);

            return materialized_source_unit{
                    .source_path = exec_path,
                    .decl_path = decl_path,
                    .exec_path = exec_path,
                    .import_root_links = std::move(import_links)};
        }
        ...
        static std::string demangle_symbol_name(std::string_view mangled) {
            if (looks_like_itanium_symbol(mangled)) {
                if (auto demangled = try_demangle_symbol_name(mangled)) {
                    return *demangled;
                }
            }
            if (auto stripped = strip_one_leading_underscore(mangled);
                stripped != mangled && looks_like_itanium_symbol(stripped)) {
        ...
        static std::optional<std::vector<analysis_symbol>> try_collect_defined_symbols(
            ...
            if (!request.import_context) {
                source_path = inputs_dir / "source.cpp";
...
#include <cstring>
    ...
    analysis_result run_analysis(const analysis_request& request, analysis_kind kind) {
        ...
        auto source_unit = detail::materialize_source_unit(request, inputs_dir, source_text);
        auto source_path = source_unit.source_path;
